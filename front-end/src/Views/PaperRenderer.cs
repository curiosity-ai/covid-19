...
using Mosaik;
using System;
using Mosaik.Helpers.UX;

namespace Covid
{
    internal class PaperRenderer : INodeRenderer, INodeReviewRenderer
    {
        ...
        public async Task<CardContent> PreviewAsync(Node node, Parameters state)
        {
            ...
            return CardContent(Header(this, node), CreateView(node), null).PreviewHeight(80.vh()).PreviewWidth(80.vw());
        }

        public async Task<IComponent> ReviewAsync(Node node)
        {
            return CardContent(Header(this, node), CreateReview(node), new Footer(Label(TextBlock("Authors:").XSmall().SemiBold().PaddingRight(8.px())).AlignCenter().Inline().SetContent(NeighborsAsLabels(node.UID, Schema.N.Author, Schema.E.HasAuthor, textSize: TextSize.XSmall)), NodeCardCommands.For(node))).Merge();
        }
                         ...
                         Pivot().Stretch()
                            .Pivot("overview", 
                                    ...
                                    () => Stack().Stretch().Children(
                                            ...
                                            fullText))
                            .Pivot("similar",
                                    () => Button("Similar").Link().Primary(), 
                                    () => SimilarSearchArea(Schema.N.Paper).OnSearch(sr =>
                                    {
        ...
        private IComponent CreateReview(Node node)
        {
            // To access properties of the node, you can strongly-typed methods such as node.GetString("stringFieldName") or node.GetInt("intFieldName")
            // For accessing the possible 'Label Fields' of the node, use: Labels.Get(node, LabelField);
            var pmcid = node.GetString("Pmcid");
            ...
            return Stack()
                    ...
                    .Children(
                        ...
                        Label("External Links: ").Inline().SetMinLabelWidth(120.px()).SetContent(Stack().Horizontal().Children(pmc, pubmed, msa, empty)),
                        Label("Full Paper: ").Inline().SetMinLabelWidth(120.px()).SetContent(Link("https://sci-hub.tw/" + node.GetString("Doi"), Button("Unlock").Link().SetIcon(LineAwesome.LockOpen)).OpenInNewTab()),
                        Label("Abstract").SetContent(CreateGradientOverlay(DocumentFromField(node, "Abstract", Mosaik.Components.DocumentFromField.Mode.Text), 30.vh()))
                        );
        }
